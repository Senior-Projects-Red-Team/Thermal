...
title("Heat Flow through Regolith with specified surface Temps")
ylabel("Heat Flow (kW)")
xlabel("Time (Days)")
hold off
...
legend(num2str(transpose(legend_arr)))
yline(0);
title("Net Greenhouse Energy with specified surface Temps")
...
yline(Q_dot/1000);
legend(num2str(transpose(legend_arr)))
title("Heat Flow through Regolith with specified Heat Resistance")
ylabel("Heat Flow (kW)")
xlabel("Time (Days)")
hold off
...
title("Net Greenhouse Energy with specified Heat Resistance")
legend(num2str(transpose(legend_arr)))
...
R_targ = 1.7; % Inspection of the graph reveals that this will allow 0 heaters.
...
figure()
hold on
title("Wall thickness with Thermal Conductivities to hit R_targ")
plot(ks, Ls)
xline(0.005)
xline(0.02)
xline(0.04)
