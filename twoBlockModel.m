...
Qdots_Structure(4) = Qdots_Structure(4) + (Ts_Structure(3) - Ts_Structure(4))/Rs_Structure(3) - (Ts_Structure(4) - Ts_Moon(1))/Rs_Structure(4); % Bridges
 ...
 % Moon
Qdots_Moon(1) = Qdots_Moon(1) + (Ts_Structure(4) - Ts_Moon(1))/Rs_Structure(4) - (Ts_Moon(1) - Ts_Moon(2))./Rs_Moon(1); % Outer Wall to 1st Layer
for i = 2:length(Rs_Moon)-1
    Qdots_Moon(i) = Qdots_Moon(i) + (Ts_Moon(i-1) - Ts_Moon(i))/Rs_Moon(i-1) - (Ts_Moon(i) - Ts_Moon(i+1))./Rs_Moon(i); % Layers of Moon
end
Qdots_Moon(end) = Qdots_Moon(end) + (Ts_Moon(end-1) - Ts_Moon(end))/Rs_Moon(end-1); % Final Layer of the Moon
...
for i = 1:3
    if (Ts_Destinations(i) < 25+273.15)
        deltaT = (25+273.15)-Ts_Destinations(i);
        reldT = deltaT / 3;
